...
void plot_dsigma_dt(std::string meson)
{
    ...
    plt::save("out/plots/dsigmadt/" + meson + "/"+ meson + "_dsigma_dt_" + timestamp() + ".png");
    plt::show();
}
...
void plot_N_models(const std::vector<std::pair<std::string,std::string>>& files, double x)
{
    ...
    std::string filename =
        "out/plots/N/N_models_" + timestamp() + "_x=" + doubleParaString(x) + ".png";
...
void plot_sigma_placeholder(std::string csv, std::string dipolemodel, std::string meson)
{
        ...
        plt::xlabel("$W$ [GeV]");
        plt::ylabel("$\\sigma$ [nb]");
        plt::title("Produção exclusiva de $\\" + meson + "$ em γ-Pb (" + dipolemodel + ")");

        std::string out =
            "out/plots/sigma/upsilon/" +
            meson + "_" +
            dipolemodel + "_" +
            timestamp() + ".png";
...
PyRun_SimpleString(cmd.c_str());
    ...
    std::filesystem::create_directories("out/plots/sigma");
    plt::save("out/plots/sigma/psi/Jpsi_gammaPb_" + dipolemodel + "_" + timestamp() + ".png");
...
std::cout << "min = " << minv << "  max = " << maxv << std::endl;
    ...
    std::filesystem::create_directories("out/plots/sigma");
    plt::save("out/plots/sigma/phi/phi_gammaPb_" + dipolemodel + "_" + timestamp() + ".png");

    plt::show();
}


void plot_sigma_gammaA(std::string meson, std::string csv, std::string modelo)
{
    if (meson == "psi"){
        ...
        plot_sigma_placeholder(csv, modelo, meson);
    }
}
...
void plot_TA_b(){
    ...
    plt::grid(true);
    plt::legend();
    plt::save("out/plots/other/TA_b(Pb-208)_" + timestamp() + ".png");
    plt::show();
...
plt::save("out/plots/overlap/sem_fc_" + meson + "_" + timestamp() + ".png");
...
plt::save("out/plots/overlap/com_fc_" + meson + "_" + timestamp() + ".png");
...
for (int flavor : flavors)
{
    ...
    py << "plt.show()\n";
    py << "plt.savefig('out/plots/xf/" << xf << "_multiQ2_" << pdf_name << "_" << timestamp() << ".png', dpi=300)\n";
...
)
{
    ...
    plt::save("out/plots/N/N_"+modelo+"_" + timestamp() + ".png");
    plt::show();
    plt::close();
}


void compare_curves(vector<string> filenames, string observable)
{
    if(observable == "sigma"){
